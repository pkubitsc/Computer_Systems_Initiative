...
class Auth extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->library('tank_auth');
                ...
                // image config variables
                $image_config['upload_path'] = FCPATH.$this->config->item('image_upload_path', 'tank_auth');
                $image_config['allowed_types'] = $this->config->item('image_allowed_types', 'tank_auth');
                $image_config['max_size'] = $this->config->item('image_max_size', 'tank_auth');
                $image_config['max_width'] = $this->config->item('image_max_width', 'tank_auth');
                $image_config['max_height'] = $this->config->item('image_max_height', 'tank_auth');
	...
	function login()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			redirect($this->config->item('login_redirect', 'tank_auth'));
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						...
						$this->form_validation->set_value('password'),
                                                ...
                                                $this->form_validation->set_value('biography'),
						$email_activation))) {									// success

					$data['site_name'] = $this->config->item('website_name', 'tank_auth');
                                        
                                        // Do image upload here (it is optional)
                                        if (isset($_FILES)) {
                                                if (!empty($_FILES['userfile']['name'])) {
                                                        if (!$this->upload->do_upload()) {
                                                            $image_errors = $this->upload->display_errors();
                                                        } else {
