...
class Users extends CI_Model
{
	...
	{
                $query = $this->db->query("SELECT users.*, user_profiles.biography, user_profiles.first_name, user_profiles.last_name, user_profiles.profile_image
                                    FROM users
                                    JOIN user_profiles ON users.id = user_profiles.user_id
                                    WHERE users.id = ".$user_id." AND
                                          users.activated=".($activated ? 1 : 0));

                //$this->db->select('users.username, users.email, user_profiles.biography, user_profiles.first_name, user_profiles.last_name, user_profiles.profile_image');
                //$this->db->from($this->table_name);
                //$this->db->join($this->profile_table_name, 'users.id = user_profiles.user_id');
		//$this->db->where('users.id', $user_id);
		//$this->db->where('users.activated', $activated ? 1 : 0);
	...
	/**
	 * Create new user record WITH a profile
	 *
	 * @param	array
	 * @param	bool
	 * @return	array
	...
	function create_user($data, $activated = TRUE)
	{
		...
		$data['activated'] = $activated ? 1 : 0;
                ...
                $new_data['biography'] = $data['biography'];
		...
		if ($this->db->insert($this->table_name, $data)) {
                        ...
                        $new_data['user_id'] = $this->db->insert_id();
                        $this->create_filled_profile($new_data);
			//if ($activated)	$this->create_profile($user_id);
			return array('user_id' => $new_data['user_id']);
		}
		return NULL;
	}
	...
	private function create_profile($user_id)
	{
        ...
        /**
	 * Create a filled profile for a user
	 *
	 * @param	int
	 * @return	bool
	 */
	private function create_filled_profile($data)
	{
	 ...
	 * @return	void
	 */
        function change_profile_image($user_id, $filename) {
