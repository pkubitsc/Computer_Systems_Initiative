...
class Follow
{
	...
	function __construct()
	{
		...
		$this->ci->load->model('tank_auth/users');
                $this->ci->load->model('home/hashtags');
                //$this->ci->load->model('home/posts');
	...
	function is_logged_in($activated = TRUE)
	{
        ...
        function change_profile_image($user_id, $data) {
                ...
                $new_filename = $user_id.$data['file_ext'];

                /* Set the rules for the resize */
                $config['image_library'] = 'gd2';
                $config['source_image']	= FCPATH.$this->ci->config->item('image_upload_path', 'tank_auth').$filename;
                ...
                rename(FCPATH.$this->ci->config->item('image_upload_path', 'tank_auth').$filename, FCPATH.$this->ci->config->item('image_upload_path', 'tank_auth').$new_filename);

                // want to create a smaller version as well
                /* Set the rules for the upload */
                $config2['image_library'] = 'gd2';
                $config2['source_image'] = FCPATH.$this->ci->config->item('image_upload_path', 'tank_auth').$new_filename;
                $config2['new_image'] = FCPATH.$this->ci->config->item('image_true_path', 'tank_auth').$new_filename;
                $config2['maintain_ratio'] = TRUE;
                $config2['width'] = $this->ci->config->item('profile_image_max_width', 'tank_auth');
                $config2['height'] = $this->ci->config->item('profile_image_max_height', 'tank_auth');;
                ...
                $this->ci->users->change_profile_image($user_id, $new_filename);
        }
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $first_name, $last_name, $biography, $email_activation)
	{
